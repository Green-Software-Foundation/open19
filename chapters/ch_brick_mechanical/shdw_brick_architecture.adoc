...
ifndef::brick[:brick: single height double wide brick]
ifndef::UBrick[:UBrick: Single height double wide brick]
ifndef::brickplus[:brickplus: single height double width (SHDW)]
:bricktype: shdw
:shdw:
:feedcount: 2
:chapter:
...
= {Ubrick} Mechanical Specification [SHDW]
...
The SHDW {brickplus} is commonly used as a 1RU server form-factor.
...
The faceplate is notched above and below the fastener (See <<#img-shdw-brick-retention>>). Note that the faceplate can accommodate any type of ports or drives as needed and is not required to be fully perforated.
